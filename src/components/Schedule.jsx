...
import { setScheduleBoolean } from "../redux/reducer";
import { useRef } from "react";
import { useEffect } from "react";

function Schedule({ DurationVideo, TimelineVideo, DeliverablesVideo }) {
  ...
  const [forDurationVideoFiles, setDurationVideoFiles] = useState([]);
  const [forTimelineVideoFiles, setTimelineVideoFiles] = useState([]);
  const [forDeliverablesVideoFiles, setDeliverablesVideoFiles] = useState([]);

  DurationVideo(forDurationVideoFiles);
  TimelineVideo(forTimelineVideoFiles);
  DeliverablesVideo(forDeliverablesVideoFiles);

  const onUpload1 = (files) => {
    setValue1(files[0].name);
    setDurationVideoFiles(files);
    setBool(true);
  };

  const onUpload2 = (files) => {
    setValue2(files[0].name);
    setTimelineVideoFiles(files);
    setBool(true);
  };
  ...
  useEffect(() => {
    ...
    if (value1 || value2 || value3 || description1 || description2 || description3) {
      dispatch(setScheduleBoolean(true));
    } else dispatch(setScheduleBoolean(false));
